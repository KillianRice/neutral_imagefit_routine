...
if avgAutoFlag % If avgAutoFlag is true (1), analyze averaged data set
    curDataset = lcl_avgDataset;
    fieldIndx = 2;
else % Else, analyze individual data set
...
%% Rabi oscillation data parameters
% Structure containing relevant fitting parameters
rabiParams = struct('state'         ,   'Excited'   ,...    % Specify which state is being measured (1=ground, 2=excited)
                    ...
                    'totNumMeas'    ,   3.3e5       ,...    % Total measured atom number (ignored if fitting number)
                    'delta'         ,   2*pi*0e3    ,...    % [s^-1] Detuning between Omega_eg and Omega_laser (ignored if fitting detuning)
                    'gammaLife'     ,   2*pi*7.5e3  ,...    % [s^-1] natural decay rate (fixed value)
                    'gammaDec'      ,   2*pi*15e3   );      % [s^-1] Best guess for decoherence rate (laser or otherwise)

%% Fitting loop
% Define whether you want to fix the detuning or the total number
fitConstr = 'fitNum'; % Valid options are 'fitDelta' or 'fitNum' 
...
fitDeltaCell     = cell(size(indVarCell));
...
for iterVar = 1:length(indVarCell)
    ...
    % Perform fitting
    [fitTime,fitGndState,fitExtState,rabiFreq,decohFreq,fitDelta,fitTotNum] = fitRabiOsc(dataTimeCell{iterVar},atomPopCell{iterVar},rabiParams,fitConstr);
    ...
    fitDeltaCell{iterVar}     = fitDelta;
    fitTotCell{iterVar}       = fitTotNum;
end

%% Plotting result
% Determine number of subplots
[nrow,ncol] = optiSubPlotNum(length(indVarCell));
...
%    totNum = fitTotCell{iterVar};
    ...
    delta_str      = sprintf('Delta. [s^{-1}]: 2\\pi \\times %5.3e Hz',fitDeltaCell{iterVar}/2/pi);
    totPop_str     = sprintf('Total Atom Num: %5.3e',fitTotCell{iterVar});
    data           = {rabiFreq_str; decohGamma_str; delta_str; totPop_str};
