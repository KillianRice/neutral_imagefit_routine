...
funcTomFerNum = @(coeffs,fields) (3/4)*(8*pi/15)*abs(coeffs.(fields{1}))/(analyVar.AbsCross)*...
    abs((coeffs.(fields{2})))*abs((coeffs.(fields{3})))*analyVar.sizefactor^2;
...
%%%%%%%-----------------------------------%%%%%%%%%%
        ...
        %winTotNum(:,k) = analyVar.sizefactor^2*sum(indivDataset{basenameNum}.All_OD_Image{k})/analyVar.AbsCross;
