...
effPixelSize = analyVar.sizefactor*analyVar.pixelconv/analyVar.pixelsize;
% 2018.05.03 - Added abs because sqrt was giving complex number sometimes and causing plots to fail
funcCloudRad = @(coeffs,fitWidth) abs(sqrt((abs(coeffs.(fitWidth)).*effPixelSize).^2 - analyVar.CameraRes.^2));
...
for basenameNum = 1:analyVar.numBasenamesAtom
    ...
    [cldRadX cldRadY]...
        = deal(NaN(sum(analyVar.LatticeAxesFit{basenameNum}),indivDataset{basenameNum}.CounterAtom));
