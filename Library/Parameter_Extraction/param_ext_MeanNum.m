...
for uniqScanIter = 1:length(analyVar.uniqScanList);
    ...
    % Function below finds the standard error of the matrix of mixed NaN's and values
    avgDataset{uniqScanIter}.stdTotNum   = nanstd(avgTotNum,0,2)./sqrt(sum(~isnan(avgTotNum),2));
    avgDataset{uniqScanIter}.stdBECNum   = nanstd(avgBECNum,0,2)./sqrt(sum(~isnan(avgBECNum),2));
    avgDataset{uniqScanIter}.stdThrmNum  = nanstd(avgThrmNum,0,2)./sqrt(sum(~isnan(avgThrmNum),2));
    avgDataset{uniqScanIter}.stdCondFrac = nanstd(avgCondFrac,0,2)./sqrt(sum(~isnan(avgCondFrac),2));
end
end