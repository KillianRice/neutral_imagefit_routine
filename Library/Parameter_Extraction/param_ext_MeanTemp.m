...
for uniqScanIter = 1:length(analyVar.uniqScanList);
    ...
    avgDataset{uniqScanIter}.stdAtomTempX = nanstd(avgAtomTempX,0,2)./sqrt(sum(~isnan(avgAtomTempX),2));
    avgDataset{uniqScanIter}.stdAtomTempY = nanstd(avgAtomTempY,0,2)./sqrt(sum(~isnan(avgAtomTempY),2));
end
end