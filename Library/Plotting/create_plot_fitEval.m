...
%%%%%%%-----------------------------------%%%%%%%%%%
        ...
        for i = 1:sum(analyVar.LatticeAxesFit{basenameNum})
...
%%%%%%%-----------------------------------%%%%%%%%%%   
        ...
        set(figStruct.fig2DFit + basenameNum,'color','white')
        fit2DAxH{basenameNum}(k) = subaxis(indivDataset{basenameNum}.SubPlotRows,indivDataset{basenameNum}.SubPlotCols,k,...
        ...
        pcolor((roiDistImage)); shading flat; axis off
        %%% Plot axis details
...
%%%%%%%-----------------------------------%%%%%%%%%%   
        ...
        set(figStruct.figRes + basenameNum,'color','white')
        resAxH{basenameNum}(k) = subaxis(indivDataset{basenameNum}.SubPlotRows,indivDataset{basenameNum}.SubPlotCols,k,...
                'SpacingVert'       ,   analyVar.subAxis.spacingVert,...
                'SpacingHoriz'  ,   analyVar.subAxis.spacingHoriz,...
                'Margin'        ,   analyVar.subAxis.margin,...
                'MarginTop'     ,   analyVar.subAxis.marginTop,...
                'Padding'       ,   analyVar.subAxis.padding);
        pcolor((indivDataset{basenameNum}.All_OD_Image{k} - roiDistImage)); shading flat; axis off
        %%% Plot axis details
        title(strcat(num2str(indivDataset{basenameNum}.imagevcoAtom(k)),[' ' analyVar.xDataUnit])); hold on; grid off;
        if k == indivDataset{basenameNum}.CounterAtom;
...
%%%%%%%-----------------------------------%%%%%%%%%%   
        ...
        set(figStruct.fig1DFit + basenameNum,'color','white')
        ...
        % Setup subplot axes
        fit1DAxH{basenameNum}(k) = subaxis(indivDataset{basenameNum}.SubPlotRows,indivDataset{basenameNum}.SubPlotCols,k,...
                'SpacingVert'       ,   analyVar.subAxis.spacingVert,...
                'SpacingHoriz'  ,   analyVar.subAxis.spacingHoriz,...
                'Margin'        ,   analyVar.subAxis.margin,...
                'MarginTop'     ,   analyVar.subAxis.marginTop,...
                'Padding'       ,   analyVar.subAxis.padding);
        hold on; grid off;
        plot(OD_1D_Xdata,'c.','MarkerSize',analyVar.markerSize); plot(OD_1D_Ydata + analyVar.condOffset,'g.','MarkerSize',analyVar.markerSize);
        plot(OD_1D_Xfit,'k');                                    plot(OD_1D_Yfit + analyVar.condOffset,'r')
        % Plot axes details
        ...
        ylim([0 max(max([OD_1D_Xdata  + analyVar.condOffset; OD_1D_Ydata' + analyVar.condOffset; ...
                         OD_1D_Xfit  + analyVar.condOffset; OD_1D_Yfit' + analyVar.condOffset]))*1.15]);
        set(gca,...
            'XTickLabel'    ,   []  ,...
            'YTickLabel'    ,   []  );
        if k == indivDataset{basenameNum}.CounterAtom;
...
%%%%%%%-----------------------------------%%%%%%%%%%
        if strcmpi(analyVar.InitCase,'Bimodal') % Same steps as above
            ...
            % Add thermal pedestal to the OD image from above
            set(0,'CurrentFigure',figStruct.fig1DFit + basenameNum); 
            set(figStruct.fig1DFit + basenameNum,'color','white')
            Thrm_1D_XFit = fitThrmDistImage(:,CloudCntr(1));              Thrm_1D_YFit = fitThrmDistImage(CloudCntr(2),:);
            plot(Thrm_1D_XFit,'k--');  plot(Thrm_1D_YFit,'r--');
            ...
            % Setup subplot axes
            fitBEC1DAxH{basenameNum}(k) = subaxis(indivDataset{basenameNum}.SubPlotRows,indivDataset{basenameNum}.SubPlotCols,k,...
                'SpacingVert'       ,   analyVar.subAxis.spacingVert,...
                'SpacingHoriz'  ,   analyVar.subAxis.spacingHoriz,...
                'Margin'        ,   analyVar.subAxis.margin,...
                'MarginTop'     ,   analyVar.subAxis.marginTop,...
